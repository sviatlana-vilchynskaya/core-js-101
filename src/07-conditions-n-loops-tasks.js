...
function isBracketsBalanced(str) {
  ...
  const stack = [];
  for (let i = 0; i < str.length; i += 1) {
    if (bracketPairs[str[i]]) {
      stack.push(str[i]);
    } else if (str[i] === bracketPairs[stack[stack.length - 1]]) {
      stack.pop();
    } else {
      return false;
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  if (pathes.length === 1) {
    const lastSlash = pathes[0].lastIndexOf('/');
    return pathes[0].slice(0, lastSlash + 1);
  }
  ...
  for (let i = 0; i < first.length; i += 1) {
    const char = first[i];
    if (others.some((item) => item[i] !== char)) {
      const lastSlash = commonPath.lastIndexOf('/');
      return getCommonDirectoryPath([commonPath.slice(0, lastSlash + 1)]);
    }
    commonPath += char;
  }
